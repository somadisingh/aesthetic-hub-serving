...
for j in range(10):
    if j < 10:
        ...
        # "pipe:aws s3 cp s3://s-datasets/laion400m/laion400m-dat-release/00625.tar -")
        dataset = wds.WebDataset(
            "pipe:aws s3 cp s3://s-datasets/laion400m/laion400m-dat-release/0000"+str(j)+".tar -")
    else:
        # "pipe:aws s3 cp s3://s-datasets/laion400m/laion400m-dat-release/00625.tar -")
        dataset = wds.WebDataset(
            "pipe:aws s3 cp s3://s-datasets/laion400m/laion400m-dat-release/000"+str(j)+".tar -")
...
for [a, b] in buckets:
    a = a/2
    b = b/2
    total_part = df[((df["prediction"]) * 1 >= a) &
                    ((df["prediction"]) * 1 <= b)]
    print(a, b)
