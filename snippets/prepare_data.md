...
```bash
# runs on node-serve-model
docker run --rm -it -v aesthetic_data:/mnt alpine sh -c "echo 'Images:' && ls /mnt/flickr-aes/40K/*.jpg | wc -l && echo 'Splits:' && ls /mnt/flickr-aes/splits/ && echo 'Test dir:' && ls /mnt/flickr-aes/test/images/ | head -5"
```

You should see ~40K images in the `40K/` folder, two CSV files in `splits/` (`flickr_global_manifest.csv` and `flickr_personalized_manifest.csv`), and symlinked images in `test/images/` and `validation/images/` for ImageFolder-based benchmarking.
